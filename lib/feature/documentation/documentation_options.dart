...
|-------|------------|-------------|---------|------|--------|----------|------------------|''',
    ...
    for (var param in apiCallInfo.params) {
      ...
      final defaultValue = entry.containsKey('default')
          ? '`${entry['default']}`'
          : '';
  ...
  void addTableHeader(
    ...
    if (info.full) {
      ...
      buffer.writeln(
        '| Name | Type | Required | Enum | Valid. | Title | Desc. | isVO |',
      );
      buffer.writeln(
        '|------|------|----------|------|--------|-------|-------|------|',
      );
  ...
  void processAttr(
    ...
    String name = split.isNotEmpty ? split.last : jsonPath;
    String isOV = '';
    if (n?.properties?['#valueObject'] == true) {
      isOV = 'Val.Obj';
    }
    ...
    if (isInArray && type == 'object') {
      ...
      if (info.full) {
        ...
        buffer.writeln(
          '| `$name` | `$typeDisplay` | $isRequired | $enumValues | $validationStr | $title | $description | $isOV |',
        );
      }
    }
    ...
    if (type == r'$ref') {
      ...
      var path = "$jsonPath>$constRefOn";
      processObject(schema, childMD, def, path, nestedRequired);
    }
