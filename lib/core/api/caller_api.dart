...
class CallerApi {
  ...
  Future<APIResponse> call(APICallManager info, CancelToken cancelToken) async {
    ...
    if (dataProviderMode.value == 'mock') {
      info.logs.add('[MOCK] this is a mock response generated');
      return await doMockAPI(url, info);
    }
  ...
  Future<APIResponse> doMockAPI(String url, APICallManager info) async {
    ModelSchema? m = await info.currentAPIResponse!.getSubSchema(subNode: 200);

    var r = Response(requestOptions: RequestOptions(path: url));
    var apiResponse = APIResponse(reponse: r);
    ...
    if (m != null) {
      ...
      r.data = {};
      apiResponse.size = r.data.toString().length;
      r.statusCode = 204;
    }

    apiResponse.contentType = ContentType.json;
    apiResponse.duration = 1;
    return apiResponse;
  }
  ...
  Future<APIResponse> callInternal(
    CallerDatasource callerDatasource,
    APICallManager apiCallInfo,
    CancelToken cancelToken,
  ) async {
    var ret = await bddStorage.getInternalData(callerDatasource, apiCallInfo);

    var responseObject = Response(
      requestOptions: RequestOptions(path: apiCallInfo.url),
    );
    var apiResponse = APIResponse(reponse: responseObject);
    responseObject.data = callerDatasource.isArray()
        ? ret
        : (ret.isEmpty ? {} : ret.first);
...
class CallScript {
  Future<dynamic> callPreRequest(APICallManager info) async {
    ...
    if (info.preRequestStr.isNotEmpty) {
      ...
      try {
        run.init(info.preRequestStr, logs: info.logs, isAsync: true);
        r = await run.eval(
          logs: info.logs,
