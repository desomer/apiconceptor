...
class _WidgetOverlySelectorState extends State<WidgetOverlySelector> {
  ...
  void initState() {
    ...
    removeAllListener(CDDesignEvent.unselect);
    ...
    on(CDDesignEvent.select, (selected) {
      ...
      if (ctx.ctx is CwWidgetCtxVirtual) {
        keybox = ctx.ctx?.parentCtx?.getBoxKey();
      }
    ...
    on(CDDesignEvent.unselect, (selected) {
      position.bottom = 0;
      position.top = 0;
      position.left = 0;
      position.right = 0;
