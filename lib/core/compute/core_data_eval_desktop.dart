...
class CoreDataEvalDart {
  ...
  void compil(String expression, List<String> logs, bool isAsync) {
    ...
    lines = _splitIgnoringStringsAndComments(expr);
    _compilProgram(lines, -1, compiler, isAsync);
    ...
    map[$String('getData')] = $Closure((runtime, target, args) {
      ...
      if (context == null) {
        ...
        return _getEvalObj(valueFromPath);
      } else {
  ...
  ResultCompil _compilProgram(
    ...
    for (var element in lines) {
      var count = _countNewlines(element);
      var nbCr = _countLeadingLineBreaks(element);
      if (element.trim().isNotEmpty) {
        debug.write(
          '_\$\$debug(${i + nbCr}, $nb );', //\'\'\'${escapeQuotes(element.trim())}\'\'\'
        );
        var analyzeLine = _removeDartComments(element).trim();
    ...
    if (!compilOk && ligne == -1) {
      ...
      ResultCompil cr;
      while (true) {
        cr = _compilProgram(lines, j, compiler, isAsync);
        if (cr.error != null) {
          break;
        }
        j = cr.idx;
  ...
  Future _loadApi(String domain, String name) async {
    ...
    return _getEvalObj(req);
  }

  Map<String, dynamic> _convertMapRecursively(Map<dynamic, dynamic> input) {
    return input.map((key, value) {
      final newKey = key.toString();
      dynamic newValue;

      if (value is Map) {
        newValue = _convertMapRecursively(value);
  ...
  String _removeDartComments(String code) {
  ...
  int _countNewlines(String input) {
    return RegExp('\n').allMatches(input).length;
  }

  int _countLeadingLineBreaks(String input) {
    int k = 0;
  ...
  List<String> _splitIgnoringStringsAndComments(String input) {
    List<String> result = [];
  ...
  $Value _getEvalObj(dynamic vr) {
    if (vr == null) {
      ...
      for (var element in vr.entries) {
        ret[$String(element.key)] = _getEvalObj(element.value);
      }
      return $Map.wrap(ret);
    } else if (vr is List) {
      var ret = [];
      for (var element in vr) {
        ret.add(_getEvalObj(element));
      }
